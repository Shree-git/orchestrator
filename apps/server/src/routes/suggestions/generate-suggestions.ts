...
The response will be automatically formatted as structured JSON.`;
  ...
  let responseText = '';
  let structuredOutput: { suggestions: Array<Record<string, unknown>> } | null =
    null;
  ...
  for await (const msg of stream) {
    if (msg.type === 'assistant' && msg.message.content) {
      ...
      if (resultMsg.subtype === 'error_max_structured_output_retries') {
        ...
        logger.error(
          'Hit max turns limit before completing suggestions generation'
        );
  ...
  try {
    if (structuredOutput && structuredOutput.suggestions) {
      ...
      logger.warn(
        'No structured output received, attempting to parse from text'
      );
